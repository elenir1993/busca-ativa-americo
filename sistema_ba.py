...
if menu == "Diagnóstico Geral":
    ...
    if st.session_state.dados_escola is not None:
        ...
        # ------------------------------------------------
        # DADOS NA NUVEM PARA RELATÓRIO E FUNIL
        # ------------------------------------------------
        todas_linhas = planilha.get_all_values()
        linha_hist = -1; hist_dados = []
        acoes_totais = {"WhatsApp": 0, "Contato Telefônico": 0, "Notificação Formal": 0, "Reunião/Visita": 0, "Conselho Tutelar": 0, "Outros": 0}
                ...
                if str(linha[0]) == "HISTORICO_SISTEMA":
                    ...
                    try:
                        ...
                        for acao in dados_aluno.get("acoes", []):
                            ...
                            elif "reunião" in nome_acao or "reuniao" in nome_acao or "visita" in nome_acao: acoes_totais["Reunião/Visita"] += 1
                            elif "conselho" in nome_acao: acoes_totais["Conselho Tutelar"] += 1
        ...
        if st.button("📄 Gerar Relatório PDF SEDUC"):
            ...
            for t, q in resumo.items():
                pdf.cell(140, 8, txt(str(t)), 1); pdf.cell(40, 8, str(q), 1, 1)
            ...
            pdf.ln(8)
            pdf.set_font("Arial", "B", 12); pdf.cell(0, 8, txt("3. Funil de Ações Realizadas (Trabalho da Equipe)"), 0, 1)
            pdf.set_font("Arial", "", 10)
            for k, v in acoes_totais.items():
                if v > 0: pdf.cell(0, 6, txt(f"- {k}: {v} intervenções registradas"), 0, 1)

            pdf.ln(5); pdf.set_font("Arial", "B", 12); pdf.cell(0, 8, txt("4. Análise Qualitativa"), 0, 1); pdf.set_font("Arial", "", 11)
            texto = analise_qualitativa if analise_qualitativa.strip() else "A análise aponta a evolução quantitativa dos dados e a mobilidade dos estudantes entre os degraus de risco, evidenciando o resultado do funil de ações da equipe escolar."
            ...
            if os.path.exists("turmas.png"): os.remove("turmas.png")

        # ------------------------------------------------
        # BOTÃO PARA EXCLUIR OU INSERIR REGISTROS (A MÁQUINA DO TEMPO COMPLETA)
        # ------------------------------------------------
        st.markdown("---")
    ...
    if ra:
        ...
        with st.expander("📞 Dados de Contato e Responsável", expanded=True):
            ...
            if email_resp and "@" in email_resp:
                ...
                texto_email = f"Prezado(a) responsável,\n\nInformamos que o(a) estudante {dados['cadastro']['nome']} está com a frequência em {freq_str}. O aluno(a) está em acompanhamento pela nossa equipe.\n\nCaso a frequência não aumente nos próximos 15 dias, o caso será encaminhado ao Conselho Tutelar.\n\nAcompanhe pela Sala do Futuro: https://saladofuturo.educacao.sp.gov.br/login-responsaveis \n\nPara dúvidas, compareça presencialmente às terças ou quintas, das 14:00 às 20:00 (procure Giovana, Elenir ou Vinicius).\n\nEquipe Gestora."
        ...
        if status_atual == "Em acompanhamento":
            ...
            with st.expander("⚠️ Encerrar Acompanhamento"):
        ...
        if dados["acoes"]:
            ...
            if col_bpdf1.button("Gerar PDF de Resumo do Prontuário"):
                ...
                pdf_al.cell(0, 8, txt(f"Turma: {dados['cadastro']['turma']} | Situação Final: {status_atual.upper()}"), 0, 1)
